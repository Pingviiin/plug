...
def find_words(text: str) -> list:
    ...
    """
    return re.findall(r"[A-ZÖÄÜÕ]{1}[a-zöäüõ]+", text)
...
def find_words_with_vowels(text: str) -> list:
    ...
    """
    return re.findall(r"[AEIOUÖÄÜÕ]{1}[a-zöäüõ]+", text)
...
def find_sentences(text: str) -> list:
    ...
    """
    return re.findall(r"\s*([A-ZÜÕÖÄ][^.!?]*[.!?]+)", text)
...
def find_words_from_sentence(sentence: str) -> list:
    ...
    """
    return re.findall(r"[\s,:;-]*([\w]+)", sentence)
...
def find_words_from_sentences_only(text: str) -> list:
    ...
    for s in sentences:
        output += find_words_from_sentence(s)
    return output
...
def find_phone_numbers(text: str) -> dict:
    ...
    """
    output = {}
    nums = re.findall(r"(\+\d{3})?\s?(\d{7,8})", text)
    for num in nums:
        if num[0] in output:
            output[num[0]] += [num[1]]
        else:
...
if __name__ == '__main__':
    ...
    print(find_words_with_vowels(
        'KanaMunaPelmeenApelsinÕunMandariinKakaoHernesAhven'))
    # ['Apelsin', 'Õun', 'Ahven']

    print(find_sentences(
        'See on esimene - lause. See on ä teine lause! see ei ole lause. Aga kas see on? jah, oli.'))

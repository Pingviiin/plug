...
def sort_dict_values(dictionary: dict) -> dict:
    ...
    """
    for flight in dictionary:
        times = dictionary[flight]
        times.sort()
        dictionary[flight] = times
...
def flights_to_destination(flights: list, destination: str) -> list:
    ...
    """
    dictionary = destinations_and_times(flights)
    sorted_flights = sort_dict_values(dictionary)
    for i in sorted_flights:
        if i == destination:
            return sorted_flights[i]
    else:
        return []
...
def flights_schedule(flights: list) -> dict:
    ...
    flight number.
    """
    dat = destinations_and_times(flights)
    times = {}
    times = {i: () for li in dat for i in dat[li] if i not in times}

    for flight in flights:
        flight = flight.split(",")
        if flight[1] in times:
            times[flight[1]] = (flight[0], flight[3])
    return times
...
def destinations_list(schedule: dict) -> list:
    ...
    dests = []
    for i in schedule:
        if schedule[i][0] not in dests:
            dests.append(schedule[i][0])
    dests.sort()
    return dests
...
def airlines_operating_today(schedule: dict, airline_names: dict) -> set:
    ...
    three_letters = []
    for i in schedule:
        three_letters.append(schedule[i][1][:3])

    output = set()
    for code in airline_names:
        if code[:3] in three_letters:
            output.add(airline_names[code])
    return output
...
def destinations_by_airline(schedule: dict, airline_names: dict) -> dict:
    ...
    output = {}
    for code in airline_names:
        if not schedule == {} or not airline_names == {}:
...
if __name__ == '__main__':
    ...
    schedule = {'08:00': ('Tallinn', 'OWL1234'), '10:35': (
        'Helsinki', 'BHM5678'), '09:00': ('Tallinn', 'OWL1235')}

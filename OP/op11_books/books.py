...
def author_book_count(library: list[Book], author: str) -> int:
    ...
    """
    return len([book for book in library if book.author == author])
...
def find_books_by_genre_and_year(library: list[Book], genre: str, year: int) -> list[Book]:
    ...
    for book in library:
        if (book.year == year) and genre in book.genres:
            correct_books.append(book)

    return sorted(correct_books, key=lambda book: (-book.sales, book.title))
...
def most_popular_author_per_century(library: list[Book]) -> dict[int, str]:
    ...
    for book in library:
        ...
        if century not in century_sales:
            century_sales[century] = {}

        if book.author in century_sales[century]:
